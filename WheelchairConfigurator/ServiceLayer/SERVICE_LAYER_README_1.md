...
ServiceLayer/
    ...
    Mapping/
        ...
        ExportMapper.cs         — DB entities → ConfigurationExportModel for PDF
```
...
UI
 └── IAppService (AppService)
         ...
         │                                 ValidateAsync()
         │
         ├── ExportMapper                → assembles ConfigurationExportModel
         │
         └── IExportFileBuilder          → Build(model) → PDF file
```
...
✅ AppService.ExportConfigurationAsync() — exports PDF via ExportLayer
```
...
- `ExportMapper` loads all components and categories in **bulk** — no N+1.
...
<ItemGroup>
    ...
    <ProjectReference Include="..\ExportLayer\ExportLayer.csproj" />
</ItemGroup>
```
